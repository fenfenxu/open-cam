...
def test_register_and_first_deploy(client):
    ...
    deployed = client.post(f"/api/models/{row['id']}/deploy", json={}).json()
...
def test_worse_model_rejected_unless_forced(client):
    ...
    client.post(f"/api/models/{live['id']}/deploy", json={})

    worse = _register(client, task_id, {
        "accuracy": 0.80, "recall": 0.70, "false_alarm_per_day": 5,
    }, "v2.pt")
    cmp = client.get(f"/api/models/{worse['id']}/compare").json()
    assert cmp["recommend_replace"] is False

    refused = client.post(f"/api/models/{worse['id']}/deploy", json={})
    assert refused.status_code == 409
    assert refused.json()["detail"]["recommend_replace"] is False

    forced = client.post(f"/api/models/{worse['id']}/deploy", json={"force": True})
...
def test_rollback_restores_previous(client):
    ...
    client.post(f"/api/models/{v1['id']}/deploy", json={})
    v2 = _register(client, task_id, {
        "accuracy": 0.95, "recall": 0.90, "false_alarm_per_day": 1,
    }, "v2.pt")
    client.post(f"/api/models/{v2['id']}/deploy", json={})

    rolled = client.post(f"/api/models/{v2['id']}/rollback", json={})
    assert rolled.status_code == 200
    body = rolled.json()
    assert body["rolled_back"] is True
    assert body["model"]["id"] == v1["id"]
    assert body["model"]["status"] == "live"
    assert client.get(f"/api/models/{v2['id']}").json()["status"] == "previous"
    ...
    client.post(f"/api/models/{lonely['id']}/deploy", json={})
    empty = client.post(f"/api/models/{lonely['id']}/rollback")
    assert empty.status_code == 400


def test_register_requires_artifact_and_task(client):
    missing = client.post("/api/models", json={
        "task_id": "nope",
    ...
    task_id = _confirm_task(client)
    no_file = client.post("/api/models", json={
        "task_id": task_id,
        "metrics": {"accuracy": 0.9, "recall": 0.8, "false_alarm_per_day": 1},
    })
...
def test_register_reads_eval_json(client):
    ...
    eval_path.write_text(
        ...
        encoding="utf-8")
    resp = client.post("/api/models", json={"task_id": task_id})
    assert resp.status_code == 200
...
def test_openapi_includes_model_version_paths(client):
    ...
    paths = spec["paths"]
    assert "post" in paths["/api/models"]
    assert "get" in paths["/api/models"]
    assert "post" in paths["/api/models/{model_id}/deploy"]
    assert "post" in paths["/api/models/{model_id}/rollback"]
