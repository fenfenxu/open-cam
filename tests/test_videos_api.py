...
def test_upload_via_videos_and_list_get(client, tmp_settings):
    ...
    got = client.get(f"/api/videos/{body['id']}")
...
def test_delete_unreferenced_video(client, tmp_settings):
    created = client.post("/api/videos",
...
def test_video_not_found(client):
    ...
    assert "视频不存在" in client.get("/api/videos/999").json()["detail"]
    assert client.delete("/api/videos/999").status_code == 404
...
def test_upload_duplicate_name_not_overwritten(client, tmp_settings):
    r1 = client.post("/api/videos", files={"file": ("a.avi", b"first", "video/avi")})
    r2 = client.post("/api/videos", files={"file": ("a.avi", b"second", "video/avi")})
...
def test_delete_video_after_camera_removed(client):
    ...
    assert cam.status_code == 201
    assert client.delete(f"/api/cameras/{cam.json()['id']}").status_code == 204
