...
def test_stopped_camera_health_is_null(client):
    ...
    got = client.get(f"/api/cameras/{cam['id']}").json()
...
def test_put_rename_while_running(client):
    ...
    try:
        ...
        session.close()
    resp = client.put(f"/api/cameras/{cam['id']}", json={"name": "新名称"})
...
def test_put_source_while_running_conflict(client):
    ...
    try:
        ...
        session.close()
    resp = client.put(f"/api/cameras/{cam['id']}", json={"source_uri": "/tmp/other.mp4"})
    assert resp.status_code == 409
    assert resp.json()["detail"] == "请先停止摄像头再修改视频源"
    assert client.get(f"/api/cameras/{cam['id']}").json()["source_uri"] == cam["source_uri"]


def test_put_source_is_immutable(client):
    """历史名：停止后源不再不可变，PUT 应 200。"""
    cam = _make_camera(client)
    resp = client.put(f"/api/cameras/{cam['id']}", json={"source_uri": "/tmp/other.mp4"})
    assert resp.status_code == 200
    assert resp.json()["source_uri"] == "/tmp/other.mp4"
    assert client.get(f"/api/cameras/{cam['id']}").json()["source_uri"] == "/tmp/other.mp4"


def test_put_source_type_is_immutable(client):
    """历史名：停止后可改 source_type。"""
    cam = _make_camera(client)
    resp = client.put(f"/api/cameras/{cam['id']}", json={
        "source_type": "rtsp", "source_uri": "rtsp://127.0.0.1:8554/x",
    })
    assert resp.status_code == 200
    body = resp.json()
    assert body["source_type"] == "rtsp"
    assert body["source_uri"] == "rtsp://127.0.0.1:8554/x"


def test_put_empty_source_uri_unprocessable(client):
    cam = _make_camera(client)
    resp = client.put(f"/api/cameras/{cam['id']}", json={"source_uri": ""})
...
def test_camera_not_found(client):
    ...
    assert client.put("/api/cameras/999", json={"name": "x"}).status_code == 404
...
def test_delete_cascades_rules_events_snapshots_keeps_uploads(client, tmp_settings):
    ...
    video = client.post("/api/cameras/upload",
...
def test_reconnect_stopped_conflict(client):
    cam = _make_camera(client)
    resp = client.post(f"/api/cameras/{cam['id']}/reconnect")
    assert resp.status_code == 409
...
def test_batch_start_partial(client):
    cam = _make_camera(client)
    resp = client.post("/api/cameras/batch/start", json={"ids": [cam["id"], 999]})
    assert resp.status_code == 200, resp.text
...
def test_batch_stop_idempotent(client):
    cam = _make_camera(client)
    resp = client.post("/api/cameras/batch/stop", json={"ids": [cam["id"]]})
    assert resp.status_code == 200
    assert resp.json()["results"][0]["ok"] is True


def test_batch_empty_ids_unprocessable(client):
    resp = client.post("/api/cameras/batch/start", json={"ids": []})
    assert resp.status_code == 422
...
def test_put_source_type_while_running_conflict(client):
    ...
    try:
        ...
        session.close()
    resp = client.put(f"/api/cameras/{cam['id']}", json={"source_type": "rtsp"})
    assert resp.status_code == 409
    assert resp.json()["detail"] == "请先停止摄像头再修改视频源"
...
def test_put_invalid_source_type(client):
    cam = _make_camera(client)
    resp = client.put(f"/api/cameras/{cam['id']}", json={"source_type": "ftp"})
    assert resp.status_code == 422
...
def test_batch_missing_ids_unprocessable(client):
    assert client.post("/api/cameras/batch/start", json={}).status_code == 422
    assert client.post("/api/cameras/batch/stop", json={}).status_code == 422


def test_reconnect_not_found(client):
    resp = client.post("/api/cameras/999/reconnect")
...
def test_batch_results_preserve_id_order(client):
    cam = _make_camera(client)
    resp = client.post("/api/cameras/batch/stop", json={"ids": [999, cam["id"]]})
    assert resp.status_code == 200
