"""方案部署表：pack_deployments 与 pack_deployment_resources。
...
def upgrade() -> None:
    ...
    if "pack_deployments" not in tables:
        ...
        op.create_index("ix_pack_deployments_pack_id", "pack_deployments", ["pack_id"])
    ...
    if "pack_deployment_resources" not in tables:
        op.create_table(
            ...
            sa.ForeignKeyConstraint(["deployment_id"], ["pack_deployments.id"]),
        )
        op.create_index("ix_pack_deployment_resources_deployment_id",
                        "pack_deployment_resources", ["deployment_id"])
...
def downgrade() -> None:
    ...
    if "pack_deployment_resources" in tables:
        op.drop_index("ix_pack_deployment_resources_deployment_id",
                      table_name="pack_deployment_resources")
        op.drop_table("pack_deployment_resources")
    if "pack_deployments" in tables:
        op.drop_index("ix_pack_deployments_pack_id", table_name="pack_deployments")
