"""员工与判定：people / person_channels / event_routings，events.verdict/assignee_id。
...
def upgrade() -> None:
    ...
    if "person_channels" not in tables:
        ...
        op.create_index("ix_person_channels_person_id",
                        "person_channels", ["person_id"])
    if "event_routings" not in tables:
        ...
        op.create_index("ix_event_routings_person_id",
                        "event_routings", ["person_id"])

    if "events" in tables:
        cols = {c["name"] for c in inspector.get_columns("events")}
        if "verdict" not in cols:
            op.add_column("events", sa.Column(
                "verdict", sa.String(16), nullable=True))
        if "assignee_id" not in cols:
            # SQLite 不支持 ALTER 加外键约束，只加列（FK 仅存在于全新建库）
            op.add_column("events", sa.Column(
