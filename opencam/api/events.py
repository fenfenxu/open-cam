...
@router.get("", response_model=list[EventOut], summary="事件列表", description="支持 camera_id / rule_type / vlm_verdict / acked / status / starred / needs_action 过滤与 limit/offset 分页，按时间倒序。")
def list_events(
    ...
    needs_action: Optional[bool] = Query(
        None, description="按是否待办过滤；不传则返回全部（含观察记录）"),
