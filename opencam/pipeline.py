...
def persist_hit(session, camera_id: int, rule, hit, snapshot_path: str | None,
    ...
    """按规则 intent 写事件。观察记 logged；告警立即开待办。Stage 2 再接策略关卡。"""
    rule_type = hit.rule_type if hit is not None else (rule.type if rule else "")
    stored = getattr(rule, "intent", None) if rule is not None else None
    intent = stored or default_intent(rule_type)
    ...
    session.add(event)
    session.commit()
    logger.info("事件落库: id=%d camera=%d type=%s intent=%s needs_action=%s",
                event.id, event.camera_id, event.type, intent, needs_action)
    return event
